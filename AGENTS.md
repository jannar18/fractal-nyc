...
Community site for Fractal NYC — a network of coliving houses, a campus, events, education, and publications in Brooklyn.

**This file is the universal rulebook — the tool-agnostic briefing every coding agent reads** (Cursor, GitHub Copilot, Claude Code, Codex, Gemini, …). It owns the workspace-wide rules: repo structure, tech stack, conventions, commands, safety rules, and the Lattice work-tracking protocol. Claude Code does not auto-load `AGENTS.md`, so `CLAUDE.md` `@import`s it.

Read at the start of every session, in this order:

1. **This file (`AGENTS.md`)** — structure, conventions, commands, safety, and the **Lattice** work-tracking protocol (§ Work Tracking).
2. **`DESIGN.md`** — the canonical design system: tokens (YAML frontmatter), color and type rules, layout rhythm, shape motifs, component specs. All styling traces back here; open the section you need before any styling work.
...
One file per route in `src/pages/`; sections compose pages; data lives in `src/data/`.

```
src/
├── App.tsx              Routes (wouter) + old-URL redirects
├── index.css            Design tokens, global type rules, semantic utilities
├── pages/               One file per route (Home, Visit, Campus, Events, Education, PoliticalClub, Publications, Story, People, Protocol)
├── components/
│   ├── sections/        Page sections (Hero, Campus, Education, …)
│   ├── house/           House banners, pennants, Mandelbrot icon
│   ├── three/           OctahedronHero 3D scene + heroNavNodes
...
├── hooks/               Shared hooks (search, archive filter, reduced-motion)
└── data/
...
See `README.md` for the full annotated tree. Routes use display-aligned slugs (`/visit`, `/education`, `/publications`); the old paths (`/neighborhood`, `/new-liberal-arts`, `/lab`) redirect. House internal `id`s stay abstract (`neighborhood`/`school`/`lab`/`forum`) and are decoupled from both slug and display name on purpose.
...
**Before opening a PR, run in this order — all must pass:**
...
- **Shared worktree:** multiple agents may work in this repo concurrently. If you encounter changes you didn't make, investigate (`git log`, `lattice list`) before touching them — never revert work you can't attribute. (See § Shared Worktree Discipline.)

## House rules

- **Mobile-first, 375px baseline — non-negotiable.** This is a mobile-first site; the primary experience is a phone screen. Every component is designed and built at phone width first; wider viewports are progressive enhancement. Tests must include mobile viewport assertions.
...
**Never run a destructive or irreversible command without explicit user approval.** Before running one, STOP and ask: *"This will [impact]. Proceed?"* — then wait for a clear yes.
...
# Branch/PR landing onto shared branches
merging or pushing to master, force-pushing any branch

# Lattice state-wiping
clearing/resetting the .lattice board or archive
```

Outward-facing actions (pushing, opening/merging PRs, deploys) also need a go-ahead unless the user already asked for them in this session. Approval in one context does not extend to the next.
...
**Each FRAC task gets its own branch and PR.** Branch from `master`, name it `frac-<id>-<slug>` (e.g., `frac-13-font-hierarchy`). Open a PR when the task reaches `review`. Do not batch multiple tasks into one branch.
...
**The PRD is the product-requirements source of truth, tracked in Lattice** (originally task `FRAC-22`; if the board was reset, re-establish its plan file from git history before relying on this check). Before and after every PR, re-read the PRD:
...
For the full CLI reference, see the `/lattice` skill. Follow-ups discovered during a session become Lattice tasks, not TODO comments.
...
When you discover something important about how this project works — a pattern, a gotcha, a convention — **do not save it to auto-memory.** Memory is per-session and per-user; future Lattice agents in other installations will never see it. Instead, append it here (or to § Common Gotchas for frontend-specific traps), or propose updating the Lattice template for universal patterns that should ship with every `lattice init`. Keep entries concise — *what* you learned and *why* it matters. The goal: every future agent, in every future installation, benefits from what you learned.
